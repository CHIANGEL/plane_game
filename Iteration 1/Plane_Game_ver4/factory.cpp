...
Enemy Factory::GenEnemy(QString kind,int speedBase,int width)
{
    if(kind == "small")
    {
        QImage Image = QImage(":images/images/enemy1.png");
        return Enemy("small", qrand()%width, 0, Image.width(), Image.height(), qrand()%5-2, qrand()%4+2+speedBase, 1, Image);
    }
    else if(kind == "mid")
    {
        QImage Image = QImage(":images/images/enemy2.png");
        return Enemy("mid", qrand()%width, 0, Image.width(), Image.height(), qrand()%5-2, qrand()%4+1+speedBase, 5, Image);
    }
    else if(kind == "big")
    {
        QImage Image = QImage(":images/images/enemy3_n1.png");
        return Enemy("big", qrand()%width, 0, Image.width(), Image.height(), qrand()%3-1, qrand()%3+1+speedBase, 20, Image);
    }
...
Explosion Factory::GenExplosion(QString kind, int X, int Y)
{
    ...
    if(kind == "small")
    {
        ...
        return Explosion(X, Y, 300, Images);
    }
    else if(kind == "mid")
    {
    ...
    else if(kind == "big")
    {
        ...
        return Explosion(X, Y, 500, Images);
    }
    else if(kind == "plane")
    {
